      ...
      <div className="collapse collapse-arrow bg-base-200 m-1">
        ...
        <div className="collapse-content">
          ...
          <table className="table-auto">
            ...
            <tbody>
              {user.myItems.map((item) => <UserItem key={`${item.itemID}`} itemName={item.itemName} itemPrice={item.itemPrice} itemQuantity={item.itemQuantity}/>)}
            </tbody>
            </table>
        </div>
      </div>
          ...
          return (
            <div key={`${member.id}-list-items`} className="collapse collapse-arrow bg-base-200 m-1">
              ...
              <table className="table-auto">
                ...
                <tbody>
                  {member.myItems?.length ? <MemberItems items={member.myItems}/> : <tr key="user-no-items"><td>No items yet</td></tr>}
                </tbody>
                </table>
              </div>
            </div>
