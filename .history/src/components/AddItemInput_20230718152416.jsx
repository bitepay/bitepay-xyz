			...
			<div className="card-body">
				...
				<div hidden={user.status !== 'READY'}>
					<div className="flex flex-col items-center">
						<input type="range" className="range" min={15} max={30} value={tip} step={.5} onChange={(e) => setTip(e.target.value)} onMouseUp={() => userUpdateTip(user, tip)} onTouchEnd={() => userUpdateTip(user, tip)}/>
						...
						{tableMembers.filter(member => member.status === 'READY').length === tableMembers.length ? (
							<button className="btn btn-info mt-4" onClick={handleSubmit}>FINAL BILL</button>
						) : (
