...
export const JoinInvite = () => {
  ...
  if (invitationTableID && invitationInviter) {
    setUser((prevUser) => ({ ...prevUser, tableID: parseInt(params.inviteTableID) }))
  } else {
    alert('You have an invalid invite link. Please join your table using the tableID via our homepage.')
    router.push(`/`)
  }

  const handleInput = (e) => setUser((prevUser) => ({ ...prevUser, username: e.target.value }))
