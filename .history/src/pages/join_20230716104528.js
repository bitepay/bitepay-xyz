...
export default function Join() {
  ...
  */
  ...
  console.log(`/join route page with searchParams: ${JSON.stringify(searchParams)}`)

  const invitationTableID = searchParams.get('tableID')
  const inviter = searchParams.get('inviter')

  console.log(`Received table invitation for tableID: ${invitationTableID} from ${inviter}`)

  const { user, setUser } = useUserSocketContext()

  if (invitationTableID && inviter) {
    console.log('able to retrive tableID and inviter')
  } else {
    window.document.alert('Invalid invitation link. Please try again.')
  }
            ...
            <div className="flex flex-col items-center justify-center">
                <div className="bg-white shadow rounded lg:w-1/3  md:w-1/2 w-full p-10 mt-16">
                  <img className="animate-bounce mx-auto" src="table-icon.svg" alt="table-icon"/>
                    ...
                    <div className="mt-8">
                        <button role="button" aria-label="create my account" className="focus:ring-2 focus:ring-offset-2 focus:ring-green-700 text-sm font-semibold leading-none text-white focus:outline-none bg-green-700 border rounded hover:bg-indigo-600 py-4 w-full">
                            JOIN TABLE
                        </button>
                    </div>
                </div>
